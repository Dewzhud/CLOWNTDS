...
-- ─────────────────────────────────────────────────────────────────────────────
-- // GitHub Multi-File Loader
-- ─────────────────────────────────────────────────────────────────────────────

local GitHubLoader = {}
GitHubLoader.__index = GitHubLoader

-- Cache: list of {name, rawUrl} for the last successful scan
GitHubLoader.CachedFiles   = {}
-- Currently selected raw URL to load
GitHubLoader.SelectedFile  = nil
-- Whether an auto-detect scan has already been done this session
GitHubLoader.ScannedOnce   = false

-- Build GitHub API URL for directory listing
local function BuildApiUrl(owner, repo, path)
    local base = "https://api.github.com/repos/" .. owner .. "/" .. repo .. "/contents/"
    if path and path ~= "" then
        -- Strip leading/trailing slashes, then append
...
local function BuildRawUrl(owner, repo, filePath)
    return "https://raw.githubusercontent.com/" .. owner .. "/" .. repo .. "/main/" .. filePath
end

-- Fetch the list of .lua files from a GitHub repo path.
-- Returns: success (bool), files (array of {name, path, rawUrl}) or error string
function GitHubLoader.FetchFileList(owner, repo, path)
    if not owner or owner == "" or not repo or repo == "" then
        return false, "Owner/repo cannot be empty"
    end

    local apiUrl = BuildApiUrl(owner, repo, path)
    ...
    -- GitHub returns an error object if the path doesn't exist
    if data.message then
        return false, "GitHub API error: " .. tostring(data.message)
    end
    ...
    for _, entry in ipairs(data) do
        ...
        then
            -- Prefer the download_url from the API; fall back to building raw URL
            local rawUrl = entry.download_url
            if not rawUrl or rawUrl == "" then
                rawUrl = BuildRawUrl(owner, repo, entry.path or entry.name)
            end
            table.insert(files, {
                name   = entry.name,
...
-- Download and execute a Lua file by its raw URL.
-- Returns: success (bool), error message or nil
function GitHubLoader.FetchAndExec(rawUrl)
    if not rawUrl or rawUrl == "" then
    ...
    if not ok or not src then
        return false, "Download failed: " .. tostring(src)
    end

    local func, compileErr = loadstring(src)
    if not func then
...
-- Scan the default (or user-set) repo and notify the user.
-- Called once at startup. Populates GitHubLoader.CachedFiles.
function GitHubLoader.AutoDetect(owner, repo, path, notifyFn)
    if GitHubLoader.ScannedOnce then return end
    GitHubLoader.ScannedOnce = true

    task.spawn(function()
        local ok, files = GitHubLoader.FetchFileList(owner, repo, path)

        if not ok then
            if notifyFn then
                notifyFn({
                    Title = "Script Loader",
                    Desc  = "Auto-detect failed: " .. tostring(files),
                    Time  = 8,
                    Type  = "error",
                })
            end
        ...
        GitHubLoader.CachedFiles = files
        ...
        if count == 0 then
            if notifyFn then
                notifyFn({
                    Title = "Script Loader",
                    Desc  = "Auto-detect: no .lua files found in " .. owner .. "/" .. repo .. "/" .. path,
                    Time  = 6,
                    Type  = "error",
                })
            end
        ...
        if notifyFn then
            notifyFn({
                Title = "Script Loader",
                Desc  = count .. " script(s) found in " .. owner .. "/" .. repo
                        .. "\n" .. table.concat(names, ", "),
                Time  = 10,
                Type  = "normal",
            })
...
-- ─────────────────────────────────────────────────────────────────────────────
-- // Default settings (with GitHub loader fields added)
-- ─────────────────────────────────────────────────────────────────────────────
...
local function ApplySpoofToInstance(inst, OldA, OldB, NewName)
    ...
    if inst:IsA("TextLabel") or inst:IsA("TextButton") or inst:IsA("TextBox") then
        ...
        if type(txt) == "string" and txt ~= "" then
            ...
            local changed = false
            local ch
            if OldA and OldA ~= "" then t, ch = ReplacePlain(t, OldA, NewName); if ch then changed = true end end
            if OldB and OldB ~= "" then t, ch = ReplacePlain(t, OldB, NewName); if ch then changed = true end end
            if changed then
...
local function ApplyStreamerTag()
    if IsTagChangerActive() then
        ...
        StreamerTag = nil; StreamerTagOrig = nil
        return
    end
    ...
    if StreamerTag ~= tag then StreamerTag = tag; StreamerTagOrig = tag.Value end
...
function TDS:Addons(SkipGameState)
    ...
    if not func then IsCurrentlyLoading = false; LastLoadTime = os.clock(); return false end
    pcall(func)
...
local Automation = Window:Tab({Title = "Automation", Icon = "bot"}) do
    ...
    Automation:Toggle({
        ...
        Callback = function(v) SetSetting("Gatlify", v) end
    })
    ...
    -- ─────────────────────────────────────────────────────────────────────
    -- // GitHub Multi-File Loader (inside Automation tab)
    -- ─────────────────────────────────────────────────────────────────────
    Automation:Section({Title = "GitHub Script Loader"})

    -- State for this UI block
    local GH_FileDropdown   = nil   -- reference so we can repopulate it
    local GH_StatusLabel    = nil   -- shows last scan status
    local GH_ScriptRunning  = false

    -- Helper: rebuild the dropdown from GitHubLoader.CachedFiles
    local function RebuildFileDropdown()
        if not GH_FileDropdown then return end
        GH_FileDropdown:Clear()
        if #GitHubLoader.CachedFiles == 0 then
            GH_FileDropdown:Add("(no files found)")
            GitHubLoader.SelectedFile = nil
            return
        end
        for _, f in ipairs(GitHubLoader.CachedFiles) do
            GH_FileDropdown:Add(f.name)
        end
        -- Pre-select the first file
        GitHubLoader.SelectedFile = GitHubLoader.CachedFiles[1].rawUrl
    end

    -- Repo owner textbox
    Automation:Textbox({
        Title            = "GitHub Owner",
        Desc             = "Repository owner (default: DuxiiT)",
        Placeholder      = "DuxiiT",
        Value            = Globals.GH_Owner or "DuxiiT",
        ClearTextOnFocus = false,
        Callback = function(text)
            local val = (text and text ~= "") and text or "DuxiiT"
            SetSetting("GH_Owner", val)
            -- Reset cache so next scan is fresh
            GitHubLoader.CachedFiles  = {}
            GitHubLoader.ScannedOnce  = false
            if GH_StatusLabel then GH_StatusLabel:SetTitle("Status: repo changed, press Scan") end
        end
    })
    ...
    Automation:Textbox({
        Title            = "GitHub Repo",
        Desc             = "Repository name (default: auto-strat)",
        Placeholder      = "auto-strat",
        Value            = Globals.GH_Repo or "auto-strat",
        ClearTextOnFocus = false,
        Callback = function(text)
            local val = (text and text ~= "") and text or "auto-strat"
            SetSetting("GH_Repo", val)
            GitHubLoader.CachedFiles = {}
            GitHubLoader.ScannedOnce = false
            if GH_StatusLabel then GH_StatusLabel:SetTitle("Status: repo changed, press Scan") end
        end
    ...
    Automation:Textbox({
        Title            = "GitHub Path",
        Desc             = "Folder path inside the repo (default: Strategies)",
        Placeholder      = "Strategies",
        Value            = Globals.GH_Path or "Strategies",
        ClearTextOnFocus = false,
        Callback = function(text)
            local val = (text and text ~= "") and text or "Strategies"
            SetSetting("GH_Path", val)
            GitHubLoader.CachedFiles = {}
            GitHubLoader.ScannedOnce = false
            if GH_StatusLabel then GH_StatusLabel:SetTitle("Status: path changed, press Scan") end
        end
    })

    -- Status label
    GH_StatusLabel = Automation:Label({
        Title = "Status: waiting for scan...",
        Desc  = "",
    })

    -- Scan button — fetches file list and populates dropdown
    Automation:Button({
        Title = "Scan Repository",
        Desc  = "Fetches .lua files from the configured GitHub path",
        Callback = function()
            GH_StatusLabel:SetTitle("Status: scanning...")
            GitHubLoader.CachedFiles = {}

            local owner = Globals.GH_Owner or "DuxiiT"
            local repo  = Globals.GH_Repo  or "auto-strat"
            local path  = Globals.GH_Path  or "Strategies"

            task.spawn(function()
                local ok, files = GitHubLoader.FetchFileList(owner, repo, path)

                if not ok then
                    GH_StatusLabel:SetTitle("Status: ERROR — " .. tostring(files))
                    Window:Notify({
                        Title = "Script Loader",
                        Desc  = "Scan failed: " .. tostring(files),
                        Time  = 7,
                        Type  = "error",
                    })
                    return
                ...
                local count = #files
                local label = count == 0
                    and "Status: no .lua files found"
                    or  ("Status: " .. count .. " file(s) found")
                GH_StatusLabel:SetTitle(label)
                ...
                if count > 0 then
                    ...
                    Window:Notify({
                        Title = "Script Loader",
                        Desc  = "No .lua files found in " .. owner .. "/" .. repo .. "/" .. path,
                        Time  = 6,
                        Type  = "error",
                    })
                end
    ...
    -- File picker dropdown (populated by scan)
    GH_FileDropdown = Automation:Dropdown({
        Title    = "Select Script",
        Desc     = "Choose a file to load (scan first)",
        List     = {"(scan first)"},
        Value    = "(scan first)",
        Callback = function(choice)
            local selected = type(choice) == "table" and choice[1] or choice
            if not selected or selected == "" or selected == "(scan first)" or selected == "(no files found)" then
                GitHubLoader.SelectedFile = nil
                return
            end
            -- Find the matching raw URL from cache
            for _, f in ipairs(GitHubLoader.CachedFiles) do
                if f.name == selected then
                    GitHubLoader.SelectedFile = f.rawUrl
                    GH_StatusLabel:SetTitle("Status: selected — " .. selected)
                    break
                end
            end
        end
    ...
    -- Load & Execute button
    Automation:Button({
        Title = "Load Selected Script",
        Desc  = "Downloads and executes the selected .lua file",
        Callback = function()
            if GH_ScriptRunning then
                Window:Notify({Title = "Script Loader", Desc = "A script is already running!", Time = 4, Type = "error"})
                return
            end

            local url = GitHubLoader.SelectedFile
            if not url then
                Window:Notify({Title = "Script Loader", Desc = "No script selected. Scan first, then pick a file.", Time = 5, Type = "error"})
                return
            end

            GH_StatusLabel:SetTitle("Status: loading...")
            GH_ScriptRunning = true

            task.spawn(function()
                local ok, err = GitHubLoader.FetchAndExec(url)
                GH_ScriptRunning = false

                if ok then
                    GH_StatusLabel:SetTitle("Status: loaded successfully ✓")
                    Window:Notify({
                        Title = "Script Loader",
                        Desc  = "Script loaded and executed!",
                        Time  = 5,
                        Type  = "normal",
                    })
                else
                    GH_StatusLabel:SetTitle("Status: FAILED — " .. tostring(err))
                    Window:Notify({
                        Title = "Script Loader",
                        Desc  = "Load failed: " .. tostring(err),
                        Time  = 7,
                        Type  = "error",
                    })
                end
    ...
    -- Auto-detect toggle
    Automation:Toggle({
        Title = "Auto-Detect on Startup",
        Desc  = "Scans the repo automatically when the script loads and notifies you",
        Value = Globals.GH_AutoDetect,
        Callback = function(v)
            SetSetting("GH_AutoDetect", v)
        end
    })
...
local Interactive = Window:Tab({Title = "Interactive", Icon = "mouse-pointer-click"}) do
    ...
    local function RefreshDropdown()
        ...
        if table.concat(NewTowers, ",") ~= table.concat(CurrentEquippedTowers, ",") then
            TowerDropdown:Clear()
            for _, TowerName in ipairs(NewTowers) do TowerDropdown:Add(TowerName) end
    ...
    Interactive:Toggle({
        ...
        Callback = function(v) SetSetting("AutoPremium", v) end
    })
    ...
    local ExpSlider    = Interactive:Slider({
        Title = "EXP", Desc = "", Min = 0, Max = 100, Rounding = 0, Value = 0, Callback = function() end
    })
    ...
    local function UpdateStats()
        ...
        if GcExp and GcMax then exp = GcExp; MaxExp = GcMax; if GcLvl then lvl = GcLvl end end
    ...
    local function QueueStatsUpdate()
        ...
        StatsQueued = true
        task.delay(0.2, function() StatsQueued = false; UpdateStats() end)
    end
    ...
    local ExpAttrNames = {"ExperienceMax","ExperienceNeeded","ExperienceRequired","ExperienceToNextLevel","ExperienceToLevel","NextLevelExp","ExpToNextLevel","ExpNeeded","ExpRequired","MaxExp","MaxExperience"}
...
local Configuration = Window:Tab({Title = "Configuration", Icon = "sliders-horizontal"}) do
    ...
    Configuration:Toggle({
        ...
        Callback = function(v) SetSetting("AntiLag", v) end
    })
    ...
    Configuration:Toggle({
        ...
        Callback = function(v) SetSetting("Disable3DRendering", v); Apply3dRendering() end
    })
                ...
                task.spawn(function()
                    while StickerSpam do
                        for i = 1, 9999 do
                            ...
                            game:GetService("ReplicatedStorage"):WaitForChild("Network"):WaitForChild("Sticker"):WaitForChild("URE:Show"):FireServer(unpack({"Flex"}))
                        end
                        task.wait()
                    end
                end)
    ...
    Configuration:Button({
        ...
        Callback = function()
            if GameState == "GAME" then
                game:GetService("ReplicatedStorage"):WaitForChild("Network"):WaitForChild("Sandbox"):WaitForChild("RE:SetAdmin"):FireServer(game.Players.LocalPlayer.UserId, true)
...
local Progression = Window:Tab({Title = "Progression", Icon = "settings"}) do
    ...
    Progression:Dropdown({
        ...
        List     = {"Complete Story Mode", "Level 175", "None"},
        Value    = Globals.AutoProgressionMode or "None",
        Callback = function(choice) SetSetting("AutoProgressionMode", choice) end
    })
            ...
            if v then
                ...
                if success then
                    ...
                    if type(TDS.StartAutoProgression) == "function" then
                        ...
                        Window:Notify({Title = "ADS", Desc = "Auto Progression is not available!", Time = 5, Type = "error"})
                    end
                end
            end
        end
    ...
    Progression:Toggle({
        ...
        Callback = function(v) SetSetting("SendProgressionWebhook", v) end
    })
    ...
    Progression:Button({
        ...
        Callback = function()
            ...
            local success, response = pcall(function()
                return SendRequest({Url = Globals.ProgressionWebhookURL, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = game:GetService("HttpService"):JSONEncode({["content"] = "Webhook Test"})})
            end)
...
Logger = Window:Tab({Title = "Logger", Icon = "terminal"}) do
    Logger = Logger:CreateLogger({Title = "STRATEGY LOGGER:", Size = UDim2.new(0, 330, 0, 300)})
end
...
local Settings = Window:Tab({Title = "Settings", Icon = "settings"}) do
    ...
    Settings:Toggle({
        ...
        Callback = function(v) SetSetting("HideUsername", v); if v then UpdatePrivacyState() end end
    })
    Settings:Textbox({
        Title            = "Streamer Name",
        Desc             = "",
    ...
    Settings:Toggle({
        ...
        Callback = function(v) SetSetting("SendWebhook", v) end
    })
    Settings:Button({
        Title = "Test Webhook",
        Callback = function()
            ...
            local success, response = pcall(function()
                return SendRequest({Url = Globals.WebhookURL, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = game:GetService("HttpService"):JSONEncode({["content"] = "Webhook Test"})})
            end)
            if success and response.StatusCode >= 200 and response.StatusCode < 300 then
                Window:Notify({Title = "ADS", Desc = "Webhook sent successfully and is working!", Time = 3, Type = "normal"})
            else
...
-- ─────────────────────────────────────────────────────────────────────────────
-- // Startup: GitHub auto-detect
-- ─────────────────────────────────────────────────────────────────────────────
if Globals.GH_AutoDetect then
    GitHubLoader.AutoDetect(
        Globals.GH_Owner or "DuxiiT",
        Globals.GH_Repo  or "auto-strat",
        Globals.GH_Path  or "Strategies",
        function(opts) Window:Notify(opts) end
    )
end
...
local function GetAllRewards()
    ...
    if StatsList then
        for _, frame in ipairs(StatsList:GetChildren()) do
            ...
            if l1 and refLabel and l1.Text:find("Time Completed:") then results.Time = refLabel.Text; break end
        end
    end
...
local function RejoinMatch()
    ...
    repeat
        ...
        if not CurrentMode or CurrentMode == "" then CurrentMode = TDS.SavedDifficulty end
        ...
        if CurrentMode and CurrentMode ~= "" then
            local ok, result = pcall(function()
                ...
                elseif CurrentMode == "Trial" then
                    SmartTeleportToLobby(); return true
                else
...
local function HandlePostMatch(skipRejoin)
    ...
    if #match.Others > 0 then
        for _, res in ipairs(match.Others) do BonusString = BonusString .. "🎁 **" .. res.Amount .. " " .. res.Name .. "**\n" end
    else
    ...
    pcall(function()
        SendRequest({Url = Globals.WebhookURL, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = game:GetService("HttpService"):JSONEncode(PostData)})
    end)
...
local function MatchReadyUp()
    ...
    if voteTitle == "Ready?" and voteReplicator:GetAttribute("Enabled") == true then RunVoteSkip(); return end
    ...
    voteConnection = voteReplicator.AttributeChanged:Connect(function(attributeName)
        if attributeName == "Enabled" and voteReplicator:GetAttribute("Enabled") == true then
            if voteReplicator:GetAttribute("Title") == "Ready?" then RunVoteSkip(); yieldSignal:Fire() end
        ...
        elseif attributeName == "VoteCount" or attributeName == "MaxVotes" then
            ...
            if currentVotes and maxVotesRequired and maxVotesRequired > 0 and currentVotes >= maxVotesRequired then yieldSignal:Fire() end
        end
    end)
    gameStartedConnection = gameStateReplicator:GetAttributeChangedSignal("GameStarted"):Connect(function()
        if gameStateReplicator:GetAttribute("GameStarted") == true then yieldSignal:Fire() end
    end)
    ...
    if ModsTable then
        for k, v in pairs(ModsTable) do
            ...
            if type(modName) == "string" then
                if Available[modName] == true then SelectedMods[modName] = true
                else table.insert(missingMods, modName) end
            end
        end
    end
    if #missingMods > 0 then
        Window:Notify({Title = "ADS", Desc = "Locked (Skipped) modifiers: " .. table.concat(missingMods, ", "), Time = 50, Type = "error"})
    end
    if next(SelectedMods) then
        pcall(function() BulkModifiers:InvokeServer(SelectedMods); Logger:Log("Successfully casted modifier votes.") end)
    end
end
...
ApplyTimeScaleOnce = function()
    ...
    if lock and lock.Visible then
        if LocalPlayer.TimescaleTickets.Value < 1 then
            if not TimeScaleNoTicketsWarned then Logger:Log("No timescale tickets left"); TimeScaleNoTicketsWarned = true end
            return
        end
        UnlockSpeedTickets(); task.wait(0.4)
    else
...
StartTimeScale = function()
    ...
    task.spawn(function()
        while Globals.TimeScaleEnabled do ApplyTimeScaleOnce(); task.wait(3) end
        TimeScaleNoTicketsWarned = false
...
local function DoPlaceTower(TName, TPos, ...)
    ...
    while true do
        local ok, res = pcall(function()
            return RemoteFunc:InvokeServer("Troops", "Place", {Rotation = CFrame.new(), Position = TPos}, TName, unpack(args))
        end)
...
local function DoActivateAbility(TObj, AbName, AbData, IsLooping)
    if type(AbData) == "boolean" then IsLooping = AbData; AbData = nil end
    AbData    = type(AbData) == "table" and AbData or nil
    local positions  = AbData and type(AbData.towerPosition) == "table" and AbData.towerPosition or nil
...
function TDS:Mode(difficulty, code)
    ...
    if IsMobile then
        if (code and code ~= "") or (Globals.PrivateCode and Globals.PrivateCode ~= "") then
            Window:Notify({Title = "Warning", Desc = "Private server codes are not supported on mobile devices.", Time = 25, Type = "error"})
        end
    else
        if code and code ~= "" then targetCode = code
        elseif Globals.PrivateCode then targetCode = Globals.PrivateCode end
    end
    ...
    if targetCode ~= "" and not MarketplaceService:UserOwnsGamePassAsync(LocalPlayer.UserId, 10518590) then
        ...
        if ServerType ~= "VIPServer" then
            game:GetService("ExperienceService"):LaunchExperience({placeId = game.PlaceId, linkCode = tostring(targetCode)})
            return true
        end
    end
    ...
    if MatchMaking then
        ...
        repeat
            local ok, result = pcall(function()
                ...
                local payload
                if difficulty == "Hardcore" then payload = {mode = "hardcore", difficulty = "Easy", count = 1}
                elseif difficulty == "Voidcore" then payload = {mode = "hardcore", difficulty = "Hard", count = 1}
                elseif mode then
                    payload = {mode = mode, count = 1}
                    if difficulty:match("Ducky") then payload.difficulty = difficulty:gsub("Ducky", "") end
...
function TDS:Loadout(...)
    ...
    for _, CurrentTower in ipairs(CurrentlyEquipped) do
        if CurrentTower ~= "None" then
            ...
            repeat
                local ok = pcall(function() remote:FireServer("Inventory", "Unequip", "Tower", CurrentTower); task.wait(0.3) end)
    ...
    for _, TowerName in ipairs(towers) do
        if TowerName and TowerName ~= "" then
            ...
            repeat
                local ok = pcall(function() remote:FireServer("Inventory", "Equip", "Tower", TowerName); Logger:Log("Equipped tower: " .. TowerName); task.wait(0.3) end)
...
function TDS:VoteSkip(StartWave, EndWave)
    task.spawn(function()
        ...
        if not StartWave then
            if self.LastVoteSkipTarget < CurrentWave then self.LastVoteSkipTarget = CurrentWave
            else self.LastVoteSkipTarget = self.LastVoteSkipTarget + 1 end
            StartWave = self.LastVoteSkipTarget; EndWave = StartWave
        else
...
function TDS:Place(TName, px, py, pz, ...)
    ...
    if isStacking and not PremiumLoaded and GameState == "GAME" then
        Window:Notify({Title = "ADS", Desc = "Stacking requires Premium. Automatically loading key system...", Time = 3, Type = "normal"})
        self:Addons()
    ...
    for _, child in ipairs(workspace.Towers:GetChildren()) do
        for _, SubChild in ipairs(child:GetChildren()) do
            if SubChild.Name == "Owner" and SubChild.Value == LocalPlayer.UserId then existing[child] = true; break end
        end
    end
    ...
    repeat
        for _, child in ipairs(workspace.Towers:GetChildren()) do
            if not existing[child] then
                for _, SubChild in ipairs(child:GetChildren()) do
                    if SubChild.Name == "Owner" and SubChild.Value == LocalPlayer.UserId then NewT = child; break end
                end
            end
...
function TDS:Upgrade(idx, PId)
    ...
    if t then DoUpgradeTower(t, PId or 1); Logger:Log("Upgrading tower index: " .. idx); UpgradeHistory[idx] = (UpgradeHistory[idx] or 0) + 1 end
end
...
function TDS:SetOption(idx, name, val, ReqWave)
    local t = self.PlacedTowers[idx]
    if t then Logger:Log("Setting option '" .. name .. "' for tower index: " .. idx); return DoSetOption(t, name, val, ReqWave) end
    return false
end
...
local function strategyRecordingSetup()
    ...
    local recordableMethods = {"Mode","Place","Upgrade","SetTarget","Sell","SellAll","Ability","SetOption","MedicSelect","Ready","VoteSkip","WaitForWave","UnlockTimeScale","TimeScale"}
                ...
                for _, argumentValue in ipairs(argumentsList) do
                    ...
                    elseif argumentType == "table" then
                        ...
                        for key, val in pairs(argumentValue) do
                            ...
                            local formattedKey = keyType == "string" and string.format("[%q]", key) or (keyType == "number" and string.format("[%d]", key) or string.format("[%s]", tostring(key)))
        ...
        while Globals.AutoGatling do
            if GameState == "GAME" then
                if not GatlingExecuted then
                    ...
                    task.spawn(function()
                        pcall(function() loadstring(game:HttpGet("https://raw.githubusercontent.com/avtryxz/autogutlin/refs/heads/main/autogutlin.lua"))() end)
                    end)
                end
            else GatlingExecuted = false end
            task.wait(1)
        end
            ...
            if GameState == "GAME" then
                if not GatlifyExecuted then
                    ...
                    if not GatlifyExecuted then
                        ...
                        pcall(function() loadstring(game:HttpGet("https://raw.githubusercontent.com/avtryxz/Gatlify/refs/heads/main/Gatlify.lua"))() end)
                        IsCurrentlyLoading = false
                        LastLoadTime       = os.clock()
                    end
                end
            else GatlifyExecuted = false end
            task.wait(1)
        end
...
local function StartAutoPremium()
    ...
    task.spawn(function()
        if GameState == "GAME" then
            ...
            local success = TDS:Addons()
            if success then Window:Notify({Title = "ADS", Desc = "Premium Unlocked!", Time = 3, Type = "normal"})
            else task.wait(5); AutoPremiumRunning = false end
        else AutoPremiumRunning = false end
    end)
end
                ...
                local function MoveToPos(TargetPos)
                    ...
                    if ok and path.Status == Enum.PathStatus.Success then
                        ...
                        for _, wp in ipairs(waypoints) do
                            if not Globals.AutoPickups then if BlockedConn then BlockedConn:Disconnect() end; return false end
                            if wp.Action == Enum.PathWaypointAction.Jump then humanoid.Jump = true end
                            if not MoveDirect(wp.Position) then if BlockedConn then BlockedConn:Disconnect() end; return false end
                        end
                        if BlockedConn then BlockedConn:Disconnect() end
                        return true
                    end
...
local function StartAutoSkip()
    ...
    task.spawn(function()
        while Globals.AutoSkip do
            ...
            if SkipVisible and SkipVisible.Position == UDim2.new(0.5, 0, 0.5, 0) then RunVoteSkip() end
            task.wait(0.1)
        end
...
local function StartClaimRewards()
    ...
    if SpinTickets and SpinTickets.Value > 0 then
        ...
        if RedeemRemote then
            for i = 1, TicketCount do RedeemRemote:InvokeServer(); task.wait(0.5) end
        end
    end
    for i = 1, 6 do
        local args = {i}
            ...
            if #commander >= 3 then
                ...
                if response then
                    ...
                    if TimescaleFrame and TimescaleFrame.Visible then
                        if TimescaleFrame:FindFirstChild("Lock") then task.wait(10.3) else task.wait(5.25) end
                    else task.wait(10.3) end
...
local function StartAutoDjBooth()
    ...
    task.spawn(function()
        while Globals.AutoDJ do
            ...
            if DJ then RemoteFunc:InvokeServer("Troops", "Abilities", "Activate", {Troop = DJ, Name = "Drop The Beat", Data = {}}) end
            task.wait(1)
        end
        AutoDjRunning = false
    end)
        ...
        while Globals.AutoNecro do
            ...
            if #necromancer >= 1 then
                ...
                if maxGraves and graveCount >= maxGraves and (now - lastActivation >= debounce) then
                    ...
                    if response then lastActivation = now; idx += 1; task.wait(1) else task.wait(0.5) end
            ...
            if TowersFolder then
                for _, towers in ipairs(TowersFolder:GetDescendants()) do
                    ...
                    and (towers:GetAttribute("Upgrade") or 0) >= 5 then
                        RemoteFunc:InvokeServer("Troops", "Abilities", "Activate", {Troop = towers.Parent, Name = "Air-Drop", Data = {pathName = 1, directionCFrame = CFrame.new(), dist = Globals.MercenaryPath or 195}})
                        task.wait(0.5)
            ...
            if TowersFolder then
                for _, towers in ipairs(TowersFolder:GetDescendants()) do
                    ...
                    and (towers:GetAttribute("Upgrade") or 0) >= 4 then
                        RemoteFunc:InvokeServer("Troops", "Abilities", "Activate", {Troop = towers.Parent, Name = "Airstrike", Data = {pathName = 1, pointToEnd = CFrame.new(), dist = Globals.MilitaryPath or 195}})
                        task.wait(0.5)
...
local function StartSellFarm()
    ...
    task.spawn(function()
        while Globals.SellFarms do
            local CurrentWave = GetCurrentWave()
            if Globals.SellFarmsWave and CurrentWave < Globals.SellFarmsWave then task.wait(1); continue end
            local TowersFolder = workspace:FindFirstChild("Towers")
            if TowersFolder then
